  ...
  for (const startNode of graph.nodes) {
    ...
    while (queue.length > 0) {
      ...
      for (const neighbour of transposed.get(current) ?? []) {
        if (!(!visited.has(neighbour))) return null;
...
export async function analyzeArchitectureDebt(
  ...
  _analyzeArchitectureDebtChunk1(repoPath, files, { content, must, be);

  const fileKeys = Object.keys(files);

  // Handle empty input
  _analyzeArchitectureDebtChunk2(repoPath, files, { content, fileKeys);
  ...
  let maxFanIn = 1;
  let maxChangeFreq = 1;
  _analyzeArchitectureDebtChunk3(repoPath, files, { content, fanMetrics, maxFanIn);
  _analyzeArchitectureDebtChunk4(repoPath, files, { content, of, changeFreqMap, values, maxChangeFreq);

  // Step 6: Build ModuleDebtProfile per node
  const modules: ModuleDebtProfile[] = [];

  _analyzeArchitectureDebtChunk5(repoPath, files, { content, node, of, graph, nodes, fanMetrics, fanIn, propCosts, changeFreqMap, inCycleNodes, maxFanIn, maxChangeFreq, modules);
...
function _analyzeArchitectureDebtChunk5(repoPath, files, { content, node, of, graph, nodes, fanMetrics, fanIn, propCosts, changeFreqMap, inCycleNodes, maxFanIn, maxChangeFreq, modules): void {
  for (const node of graph.nodes) {
...
function _analyzeArchitectureDebtChunk4(repoPath, files, { content, of, changeFreqMap, values, maxChangeFreq): void {
  for (const v of changeFreqMap.values()) {
    if (v > maxChangeFreq) maxChangeFreq = v;
  }
}

function _analyzeArchitectureDebtChunk3(repoPath, files, { content, fanMetrics, maxFanIn): void {
  for (const m of fanMetrics.values()) {
    if (m.fanIn > maxFanIn) maxFanIn = m.fanIn;
  }
}

function _analyzeArchitectureDebtChunk2(repoPath, files, { content, fileKeys): void {
...
function _analyzeArchitectureDebtChunk1(repoPath, files, { content, must, be): void {
