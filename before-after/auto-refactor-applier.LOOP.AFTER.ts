...
function applyIntroduceParameterObjectFull(code: string, result: AutoRefactorResult): ApplyResult {
  ...
  const parenOpen = fullSig.indexOf('(');
  const parenClose = fullSig.lastIndexOf(')');
  ...
  const raw = fullSig.slice(parenOpen + 1, parenClose);
  ...
  const iface = [
    ...
    ...parsed.map(p => `${baseIndent}  ${p.replace(/^\.\.\./, '')}: unknown;`),
    `${baseIndent}}`,
    '',
  ];

  const sigLines = fullSig.split('\n');
  const sigEnd = fnStart + sigLines.length - 1;
  const firstSigLine = lines[fnStart];
  ...
  const newSigLine = `${indent}${prefix}${asyncP}function ${fnName}(params: ${ifaceName}) {`;
...
function getFnParams(sigLine: string): string[] {
  ...
  if (rest.includes(')')) {
    return rest.slice(0, rest.indexOf(')')).split(',').map(p => {
      const s = p.trim();
      const c = s.indexOf(':');
      const e = s.indexOf('=');
      const n = (e > 0 && e < (c > 0 ? c : s.length)) ? s.slice(0, e).trim() : c > 0 ? s.slice(0, c).trim() : s;
      return n.replace(/^\.\.\./, '');
    }).filter(Boolean);
  }
  return [];
...
function getFnSig(lines: string[], fnLine: number): string {
  let sig = '';
  let depth = 0;
  let foundOpen = false;
