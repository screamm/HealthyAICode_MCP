...
## Increment 2026-06-02 — OCHS spec-härdning (sista autonoma steget)

**Ton:** Senior, brutalt ärlig, noll hype. Detta är det **sista autonoma incrementet** i den här körningen. Det angriper exakt den svaghet förra incrementet lokaliserade (§ OCHS som verklig standard, punkt 0–1): två oberoende implementationer höll med om *formeln* exakt men bara om **33 % av fynden**, därför att specen under-specificerade ~20 firing-trösklar, hade en intern motsägelse (`ComplexMethod`), och saknade 9 biomarkörer helt. Frågan detta increment besvarar: **kan OCHS göras till en spec en tredjepart faktiskt kan reimplementera på sin strukturella kärna — eller är detektionen i grunden inte reproducerbar?** Svaret är mätt, oberoende re-kört, och redovisas oförskönat. **Detta gör oss inte världsbäst och gör inte OCHS adopterad** — det gör den, för strukturella biomarkörer, *self-sufficient och tredjeparts-reimplementerbar*, vilket den bevisligen inte var igår.

### 0. Det avgörande, oberoende re-körda talet — konvergerade cross-impl-agreementet till full L2 på den strukturella delmängden?

Detektor-källan är ground truth. Varje publicerad tröskel nedan är läst ur `packages/core/src` (inte uppfunnen), och resultatet är re-mätt mot den byggda motorn — jag byggde om `core_reference.json` från live-`analyzeCode` och körde `ochs-ref-py/cross_check.py` på de 15 delade Python-fixturerna helt på nytt. **Inga fabricerade tal.**

| Mått (strukturell delmängd) | Före härdning (förra incrementet) | EFTER härdning (denna körning, oberoende re-kört) |
|---|---|---|
| Exakt score-agreement (\|Δ\| ≤ 0,0001) | 33,3 % (5/15) | **100 % (15/15)** |
| Inom-0,5-agreement | 80,0 % (12/15) | **100 % (15/15)** |
| Kategori-agreement (green/yellow/red) | 80,0 % (12/15) | **100 % (15/15)** |
| Medel \|Δ\| | 0,5303 | **0,0000** |
| L2-konformans (exakt på alla strukturfixturer) | Nej | **JA** |

**Den ärliga läsningen: trösklarna VAR hela historien för strukturell detektion.** På den strukturella delmängden producerar de två implementationerna nu **byte-identiska smell-vektorer och identiska OCHS-poäng på varje fixture**. Det är ett kvalitativt språng från igår: clean-room-Python-impl:en når nu **genuin L2** (exakt score-agreement på alla strukturfixturer), inte bara "formel-konsistent på sina egna fynd". Den tidigare divergensen berodde uteslutande på under-specificerade trösklar — inte på någon inneboende icke-reproducerbarhet, och inte på kopiering (Python-impl:en importerar fortfarande noll motorkod; enda träffen på grep är en proveniens-kommentar).

**Att det inte är en trivialitet:** fixturerna tränar faktiskt de relevanta detektorerna — `ComplexMethod`, `DeepNesting`, `CognitiveComplexity`, `BrainMethod`, `MagicNumber`, `PrimitiveObsession`, `BumpyRoad`, `DuplicateCode`, `LongParameterList`, `LowDocCoverage`. Den kontradiktions-relevanta `ComplexMethod` (CC>10-firing-beslutet) är direkt utövad och håller med. 100 %-siffran är substantiell, inte en artefakt av trivialfixturer.

### 1. Vad som nu är gjort — spec-trösklar, `ComplexMethod`-resolution, governance, conformance-CI

**Spec-härdningen (`docs/ochs/OCHS-v0.1.md`, +239 rader, noll detektorfiler rörda).** Allt nedan är extraherat ur detektorkällan och de centrala värdena empiriskt re-verifierade mot motorn:

- **Ny §2.1 (Tabeller 1a–1g): exakta numeriska firing-trösklar.** Per-funktion (`detector.ts`-defaults), fil-nivå, Tier-A design/control-flow/composite, SATD, och index-baserade maintainability-biomarkörer. Strikta jämförelser genomgående. Nyckelrader, verifierade mot källan: `ComplexMethod` `CC > 10` (`critical` vid `> 20`); `DeepNesting` `> 3` (`high` `> 4`, `critical` `> 5`); `LargeMethod` `> 50`; `LongParameterList` `> 4`; `CognitiveComplexity` `> 15` (`crit > 25`); `GodClass` `ATFD > 5 ∧ WMC ≥ 20 ∧ LCOM4 > 1`; `MagicNumber` tillåten mängd `{0,1,−1,2,100}`, per-funktion `≥ 3`, testfiler skippas; `LowMaintainability` MI-formel `171 − 5,2·ln(HV) − 0,23·C̄C − 16,2·ln(LOC)`, klamp/runda, floor 1; `BumpyRoad` `≥ 4` chunks. Detta är det enda arbete som faktiskt kunde lyfta cross-impl-agreement från 33 % mot full L2 — och det gjorde det.
- **§2.2 löser `ComplexMethod`-motsägelsen — genuint, inte kosmetiskt.** Den gamla "1.0 vid CC 15–24, 1.5 vid CC ≥ 25"-formuleringen är **rättad och motbevisad mot ground truth**: `weights.ts` har en enda konstant `ComplexMethod: 1.5`; `scorer.ts` är en per-*typ*-vikt (`score −= weight × √count`), så en "1.0-tier" är **strukturellt orepresenterbar** i formeln — exakt som §2.2 nu argumenterar. De föräldralösa 15/25-talen är korrekt om-tilldelade till den separata `CognitiveComplexity` (vikt 0,8, `>15`/`>25`). Ingen residual motsägelse; round-then-floor-regeln i §2.4 matchar scorern exakt.
- **§2.3 publicerar `DuplicateCode`-algoritmen** (minLines=6, `<12` rader fire aldrig, SHA-1-fönsterhash, type-1/type-2-normalisering, region-merge, logic-content-guard) — en av de biomarkörer som tidigare inte gick att reimplementera ur prosan, nu fullt specificerad.
- **§2.4 låser rundningsregeln** (`round1` före floor) som krävs för byte-exakt determinism.
- **Appendix D — den ärliga annexen.** De 55 biomarkörerna partitioneras nu rent: **25 strukturella / spec-reproducerbara** (§2.4) + **30 annexerade / inte-spec-reproducerbara** (D.1 registry-data: `HallucinatedPackageImport`, `SlopsquattingRisk`, `DependencyVulnerability`; D.2 git/historik: `MethodTemporalCoupling`, `CodeChurn`, `DeveloperCongestion`, `KnowledgeLoss`, `ArchitectureDebt`, `TestProximity`; D.3 curated-pattern/SDK-lista: säkerhets-sink-, secret-, LLM-, async/exception-anti-pattern-heuristikerna) = 55, ingen överlapp. **Detta är hederslöftet infriat:** för D.1/D.2 publiceras inget falskt spec-derivat — de märks "kräver registry-snapshot / repo-historik / delad curated-lista". För D.3 publiceras *algoritmen* men det sägs rakt att *listan* är detektorn. Att utelämna en annexerad biomarkör är konformant (formeln oförändrad, fyndet bidrar bara 0). **Ingen påstådd spec-härledbarhet för något som inte är det.**
- **Governance + conformance-CI** (nya filer, från det parallella governance-spåret): `SECURITY.md` (zero-egress-posture, GitHub Security Advisories-kanal, 90-dagars coordinated disclosure), `MAINTAINERS.md` (med explicit "single-maintainer, single-org — känd lucka mot OpenSSF Incubating"-not), `LICENSE-CC-BY-4.0.md` (dokumentationslicens vid sidan av MIT-koden), och `.github/workflows/ochs-conformance.yml` (4 jobb: referens-impl-conformance, `ochs-validate`-CLI-smoke, JSON-schema-meta-validering via ajv, typecheck). `LICENSE` (MIT) var redan korrekt och rördes inte.

### 2. Den definitiva autonoma-runway-slutsatsen — den autonoma ingenjörsfasen är KLAR

Detta måste sägas rakt och slutligt: **den autonoma ingenjörsfasen för win-by-margin är härmed slut.** Inte för att allt är gjort, utan för att **varje återstående steg som faktiskt avgör om vi vinner med marginal kräver ägaren (David Rydgren, @screamm) personligen** — konton, identitet, förhandlingsmandat och relationer som ingen agent kan tillhandahålla. Den autonoma loopen har nått sitt kod-tak: OCHS är nu tekniskt self-sufficient på sin strukturella kärna, conformance-kit + validator + CI finns, de tre stödjande artefakterna (behavior-equiv-grind, slopsquatting, gate-FP) är byggda och mätta, och pilot-RCT:erna är körda och ärligt nedjusterade. Det finns inget kvar att *bygga* autonomt som flyttar marginalfrågan.

De återstående stegen — **alla OWNER-gated, inget av dem autonomt görbart:**

1. **Lämna faktiskt in OCHS till OpenSSF (Sandbox) och/eller OWASP (Incubator)** och **säkra en TAC-sponsor.** Kräver OpenSSF-Slack-#tac-outreach eller bi-veckomöte (2–6 v), forka `ossf/tac`, skriva lifecycle-dokument, trigga LF IP-review. Ingen agent kan skaffa en sponsor.
2. **Rekrytera ≥2 externa implementatörer/adopters + ≥2 maintainers från ≥2 organisationer.** Detta är relations- och förtroendearbete (OpenSSF Incubating-krav; vi har 1 maintainer, 1 org). Det är den enda saken som förvandlar "andra clean-room-impl byggd av samma projekt" till "extern adoption" — och det är vad som skiljer en standard från ett välbeskrivet format.
3. **Publicera `ochs-validate` till npm/PyPI under ägarkontot + outreach** (dev.to/OpenSSF-blogg, bestpractices.dev-registrering) i ägarens namn.
4. **Kör den fullständiga förregistrerade riktiga-agent-RCT:n och fatta beslutet** — via `runArmWithLlm()`-vägen, en levande agent som redigerar källfiler i en loop mot riktiga issues, med ett uppgiftsurval som framkallar riskfyllda refaktoreringar; publicera rådata oavsett utfall. (Tre pilotmätningar har redan nedjusterat förväntan: +0,296, +0,167, 68 % real-fil-verifierbarhet på gynnsam korpus.)
5. **Skaffa riktiga användare** för data-flywheelen (Spel 6) — ingen agent kan etablera adoption.
6. **Eftersträva peer-review** (SANER/MSR/FORGE 2027 för behavior-equiv-grinden; NeurIPS D&B för HealthBench) och rekrytera en akademisk medförfattare.

### 3. Den ärliga bottenraden om "bäst i världen"

**Vad vi har byggt och bevisat (autonomt, verifierat):** en deterministisk, öppen, dubbelt-implementerad kod-hälsostandard vars **strukturella kärna (25/55 biomarkörer) en oberoende clean-room-impl nu reproducerar byte-exakt** (100 % L2 på 15 fixturer) — med de 30 reference-data-beroende biomarkörerna ärligt annexerade snarare än överclaima-de; ett conformance-kit (L1/L2/L3, 123/123 tester) + fristående validator-CLI; en blockerande behavior-equivalence-grind för den isolerbara Python/TS/JS-delmängden (90 %/0 % FP på 35 hand-par, 68 % real-fil-verifierbarhet); ett verifierat slopsquatting-claim (100 %/100 % på märkt mängd, 0-nätverk vid opt-out); och en delta-gate med 0 % FP. Allt grönt: typecheck 5/5 paket, `ochs-validate` 123/123, cross-impl oberoende re-kört.

**Det oöverbryggbara-för-en-agent-gapet:** "bäst i världen" avgörs inte av kod. Det avgörs av (a) **adoption** — en extern part som emitterar en OCHS-konform score och ≥2 maintainers från ≥2 organisationer; (b) **ett kausalt RCT-resultat med en riktig agent** publicerat och reproducerat; (c) **peer-review**; och (d) **riktiga användare**. Inget av dessa kan en autonom loop producera — de kräver mänskliga konton, institutionell tillit och tid. Vi har byggt artefakten och gjort den submission-redo; vi kan inte trycka på "skicka", och vi kan inte få en tredjepart att anta den.

**Det enskilt viktigaste ägaren bör göra FÖRST:** säkra en OpenSSF-TAC-sponsor och lämna in OCHS Sandbox-förslaget. Inte för att det är mest arbete, utan för att det är **grindstenen som låser upp resten** — en neutral governance-hemvist är det som förvandlar OCHS från "ett vänligt namngivet format med två implementationer från samma projekt" till infrastruktur en konkurrent strukturellt inte kan följa efter på (anti-Terraform-draget, gjort *före* skala). Allt annat — adopters, maintainers, peer-review, RCT-citeringar — följer lättare när standarden har ett neutralt hem; ingenting följer alls om den förblir ett ensamt repo. Den tekniska self-sufficiency som krävdes för att en sponsor ska kunna ta förslaget på allvar är nu på plats. Nästa drag är ägarens, och det är ett människodrag.

**Sammanfattat:** detta sista autonoma increment konvergerade cross-impl-agreementet till 100 % på den strukturella kärnan, löste `ComplexMethod`-motsägelsen mot ground truth, annexerade de 30 reference-data-biomarkörerna ärligt, och landade governance/CI-artefakterna. Det gör OCHS tredjeparts-reimplementerbar och submission-redo — och det markerar slutet på vad en agent kan göra. Marginalen vinns nu, om den vinns, av ägaren: filing, adoption, RCT-beslutet, peer-review och riktiga användare. Vägen är intakt och tydligare än någonsin; runwayn för autonomt arbete är slut.
