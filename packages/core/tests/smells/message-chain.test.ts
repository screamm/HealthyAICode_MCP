...
describe('MessageChain', () => {
  it('flags chain of depth 4', () => {
    ...
    expect(detectMessageChain(tree, typescriptProfile).some(s => s.type === 'MessageChain')).toBe(true);
  });

  it('does not flag chain of depth 3', () => {
    expect(detectMessageChain(parse('const r = a.b().c().d();'), typescriptProfile)).toHaveLength(0);
  });

  it('does not flag simple method call', () => {
    expect(detectMessageChain(parse('foo.bar();'), typescriptProfile)).toHaveLength(0);
  });
});
