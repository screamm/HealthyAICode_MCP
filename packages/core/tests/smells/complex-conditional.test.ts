...
describe('ComplexConditional', () => {
  it('flags nested ternary as high', () => {
    ...
    expect(detectComplexConditional(tree, typescriptProfile).some(s => s.severity === 'high')).toBe(true);
  });

  it('flags boolean chain > 3 as medium', () => {
    const tree = parse('if (a && b && c && d) {}');
    expect(detectComplexConditional(tree, typescriptProfile).some(s => s.severity === 'medium')).toBe(true);
  });

  it('does not flag simple ternary', () => {
    expect(detectComplexConditional(parse('const x = a ? b : c;'), typescriptProfile)).toHaveLength(0);
  });

  it('does not flag short boolean chain', () => {
    expect(detectComplexConditional(parse('if (a && b) {}'), typescriptProfile)).toHaveLength(0);
  });
});
