...
// each point of improvement yields ~4% faster development and ~4% lower defect rate.
...
function successResponse(filePath: string, result: HealthResult) {
  ...
  const businessCase = buildBusinessCase(result.score, improvement);
  const body = {
    filePath,
...
function buildBusinessCase(currentScore: number, improvement: number): BusinessCase {
  ...
  return {
    ...
    aiReadinessGain: aiReadinessLabel(currentScore),
    recommendation: recommendationLabel(improvement, speedGainPct),
  };
}

function aiReadinessLabel(currentScore: number): string {
  return currentScore < TARGET_SCORE
    ? 'Koden är inte AI-redo. Refaktorering möjliggör säker AI-assistans.'
    : 'Koden är redan AI-redo.';
}

function recommendationLabel(improvement: number, speedGainPct: number): string {
  if (improvement > HIGH_PRIORITY_GAIN) {
    return `Hög prioritet: ${improvement.toFixed(1)} poängs förbättring ger ${speedGainPct}% snabbare leverans`;
  }
  if (improvement > MEDIUM_PRIORITY_GAIN) {
    return `Medium prioritet: ${improvement.toFixed(1)} poängs förbättring ger marginella förbättringar`;
  }
  return 'Låg prioritet: Filen är redan i gott skick';
}
